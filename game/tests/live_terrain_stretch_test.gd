...
func _run() -> void:
	...
	var bytes: PackedByteArray = main.document_state.current_document.serialize().data
	var random_before: int = main.tool_state.tool_random.state
	_begin()
	_motion(-36, false)
	...
	_motion(-24, true)
	assert(main.document_state.current_document.serialize().data == bytes, "Shift drag changed terrain before release")
	_motion(-24, false)
	assert(main.document_state.current_document.serialize().data != bytes)
	main.map_view.cancel_active_selection()
	assert(main.document_state.current_document.serialize().data == bytes)
	assert(main.tool_state.tool_random.state == random_before)
	_begin()
	_motion(-36, true)
	assert(main.document_state.current_document.serialize().data == bytes)
	_release(-36, true)
	assert(main.document_state.current_document.serialize().data != bytes, "Shift drag did not change terrain on release")
	assert(not main.tool_state.terrain_stretch.active)
	assert(TerrainCommand.undo(main.document_state.city, main.tool_state.last_edit_command, main.tool_state.tool_random).ok)
	assert(main.document_state.current_document.serialize().data == bytes)
	_begin()
	_motion(-24, false)
	_motion(0, false)
	_release(0, false)
	assert(main.document_state.current_document.serialize().data == bytes, "Tile stayed raised after the drag returned to its origin")
	_begin()
	_motion(-24, false)
	_release(-24, false)
	assert(main.document_state.current_document.serialize().data != bytes)
	assert(TerrainCommand.undo(main.document_state.city, main.tool_state.last_edit_command, main.tool_state.tool_random).ok)
	assert(main.document_state.current_document.serialize().data == bytes, "Undo did not restore the whole live drag")
...
func _wait_for_render() -> void:
	for frame in 300:
		...
		if main.render_caches.static_city_image != null and main.static_render_state.task == null:
			return
