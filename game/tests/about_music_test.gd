...
func _run() -> void:
	...
	main._load_city_unchecked(reference.path_join("CITIES/CAPEQUES.SC2"))
	assert(main.city != null and main.city.set_music_enabled(false))
	audio.stop_music()
	main._open_about_dialog()
	assert(audio.current_track_id == -1, "The city's disabled Music option is respected")
	main.about_dialog.hide()
	assert(main.city.set_music_enabled(true))
	main._open_about_dialog()
