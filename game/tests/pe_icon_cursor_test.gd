	...
	for cursor in [false, true]:
		for bits in [1, 4, 8]:
			...
			for change in ["header", "width", "height", "odd_height", "planes", "bits", "compression", "palette", "index"]:
				...
				match change:
					"header":
						bad.encode_u32(at, 12)
					"width":
						bad.encode_u32(at + 4, 0xffffffff)
					"height":
						bad.encode_u32(at + 8, 0xfffffffc)
					"odd_height":
						bad.encode_u32(at + 8, 3)
					"planes":
						bad.encode_u16(at + 12, 2)
					"bits":
						bad.encode_u16(at + 14, 32)
					"compression":
						bad.encode_u32(at + 16, 1)
					"palette":
						bad.encode_u32(at + 32, 257)
					"index":
