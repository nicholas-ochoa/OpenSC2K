...
func _run() -> void:
	...
	assert(values.default_organization_name.is_empty())
	assert(AppSettingsStore.save_values(0.5, 0.5, false, path, "", "", null, null, null, null, null, null, null, null, null, null, "Alex", 2, "Harbor Authority") == OK)
	var updated := AppSettingsStore.load_values(path)
	assert(updated.default_mayor_name == "Alex")
	assert(updated.default_organization_name == "Harbor Authority")
	...
	dialog.default_organization_edit.text = updated.default_organization_name
	dialog.overview_graphics_selector.select(updated.overview_graphics)
	assert(dialog.selected_values().default_mayor_name == "Alex")
	assert(dialog.selected_values().default_organization_name == "Harbor Authority")
