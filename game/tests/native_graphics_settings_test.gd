...
func _run() -> void:
	...
	assert(AppSettingsStore.save_values(0.5, 0.5, false, path, "", "", null, null, null, null, null, null, null, null, null, null, "Harbor Authority", 2) == OK)
	var updated := AppSettingsStore.load_values(path)
	assert(updated.default_mayor_name == "Harbor Authority")
	...
	assert(dialog.selected_values().default_mayor_name == "Harbor Authority")
