...
func fixture(edge: int, native: bool) -> Dictionary:
	var doc := EmptyCityTemplate.create(edge)
	if native:
		doc.enable_full_resolution_maps()
	var p := GrowthPhase._duplicate_payloads(GrowthPhase._payloads(CityState.from_document(doc)))
	p["altitudes"] = CityState.from_document(doc).altitude_words.duplicate()
	p.XZON.fill(7)
