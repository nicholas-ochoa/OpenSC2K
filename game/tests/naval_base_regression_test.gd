...
func _initialize() -> void:
	for edge in Sc2File.MAP_SIZES:
		for native in [false, true]:
			for direction in 4:
				var doc := fixture(edge, native, direction)
				...
				doc = fixture(edge, native, direction)
			...
			# Exclusions use one orientation; the positive cases cover all four.
			for blocked in ["ocean", "fresh", "occupied", "underground", "slope"]:
				var doc := fixture(edge, native, 0)
				var city := CityState.from_document(doc)
				if blocked == "ocean":
...
func fixture(edge: int, native: bool, rotation: int) -> Sc2File:
