...
func _initialize() -> void:
	...
	for edge: int in [128, 256, 384, 512]:
		for group in [6, 7, 3]:
			...
			assert(preview_city.document.serialize().data == city.document.serialize().data, "Bridge preview matches the complete placed route")
			assert(NetworkCommand.undo(city, command).ok)
			assert(city.document.serialize().data == before, "One Undo restores both banks and the bridge")
		for bridge in [HighwayCommand.BRIDGE_HIGHWAY, HighwayCommand.BRIDGE_REINFORCED]:
			...
			assert(proposal.get("connection_selection_required", false))
			assert(city.document.serialize().data == before, "Connection prompt committed earlier route segments")
		...
		assert(NetworkCommand.undo(city, multi).ok and city.document.serialize().data == before)
		...
		assert(city.building_id(base + 8, 20) == 0 and limited.has("continuation_error"))
