...
func _run() -> void:
	...
	dialog.show_values(0.5, 0.3, false, "auto", "", "Original", folder, "/missing/OST")
	assert(dialog.selected_values().soundtrack_folder == folder)
	assert(dialog.soundtrack_status.text.begins_with("19 of 19"))
	dialog.soundtrack_dialog.dir_selected.emit("/missing/OST")
	assert(dialog.soundtrack_status.text.begins_with("0 of 19"))
	dialog.soundtrack_dialog.dir_selected.emit(folder)
	assert(dialog.selected_values().soundtrack_folder == folder)
