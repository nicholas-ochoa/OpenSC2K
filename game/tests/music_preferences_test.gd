...
func _run() -> void:
	...
	assert(audio.music_gap_remaining_msec == 15000.0 and not audio.dummy_music_active)
	audio.handle_application_focus_out()
	audio.advance(16000.0)
	assert(audio.music_gap_remaining_msec == 15000.0)
	audio.handle_application_focus_in(true)
	audio.advance(14999.0)
