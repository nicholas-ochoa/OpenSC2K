...
func run() -> void:
	...
	main.settings_dialog.get_node("%ImportButton").pressed.emit()
	assert(dialog.visible and not main.settings_dialog.visible)
	...
	assert(not dialog.visible and main.settings_dialog.visible)
