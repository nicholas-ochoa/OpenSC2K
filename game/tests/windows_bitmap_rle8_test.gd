...
func _initialize() -> void:
	...
	for change in ["top_down", "palette", "truncated", "size", "planes"]:
		...
		match change:
			"top_down": bad.height = 0xffffffff - 24
			"palette": bad.color_count = 257
			"truncated": bad.bytes.resize(1040)
			"size": bad.bytes.encode_u32(20, 0xffffffff)
			"planes": bad.bytes.encode_u16(12, 2)
