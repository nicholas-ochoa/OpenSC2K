...
func _run() -> void:
	...
	for edge in [64, 512]:
		...
		var cost: int = main.tool_state.last_edit_command.cost
		map._process(0.5)
		assert(main.tool_state.last_edit_command.cost == cost, "Holding over an empty tile charged again")
		...
		assert(main.tool_state.last_edit_command.cost == 11 and main.tool_state.last_edit_command.action_count == 11)
		...
		assert(main.tool_state.tool_random.state != rng and main.tool_state.last_edit_command.cost > 0)
		assert(DemolishCommand.undo(main.document_state.city, main.tool_state.last_edit_command, main.tool_state.tool_random).ok)
		assert(DocumentState.capture(main.document_state.current_document) == before and main.tool_state.tool_random.state == rng)
		...
		assert(main.tool_state.last_edit_command.underground_view and main.tool_state.last_edit_command.cost == 4)
		assert(DemolishCommand.undo(main.document_state.city, main.tool_state.last_edit_command, main.tool_state.tool_random).ok)
		assert(DocumentState.capture(main.document_state.current_document) == before and main.tool_state.tool_random.state == rng)
