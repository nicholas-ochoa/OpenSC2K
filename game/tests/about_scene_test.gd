...
func _run() -> void:
	...
	assert(dialog.project_text.text.to_lower().contains("mit license"))
	assert(dialog.project_text.text.contains("Electronic Arts"))
	assert(dialog.project_text.text.contains("sc2kfix"))
	assert(dialog.license_documents.size() == 13)
	for document in dialog.license_documents:
		assert(document.length() > 100)
	assert(dialog.license_documents.any(func(text: String) -> bool: return text.contains("MPL-2.0")))
	assert(dialog.license_documents.any(func(text: String) -> bool: return text.contains("CC BY-SA 4.0")))
