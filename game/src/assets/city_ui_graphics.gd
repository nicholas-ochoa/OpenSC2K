...
func _load(value: Variant, read_png: Callable, palette: Sc2Palette) -> void:
	if not value is Dictionary or value.is_empty():
		error = "city_ui must contain controls or hourglass"
		return
	for group in value:
		if group not in ["controls", "hourglass"]:
			error = "Unknown city_ui field: %s" % group
			return
		var ids: Array = CONTROL_SIZES.keys() if group == "controls" else HOURGLASS_IDS
		...
		for i in ids.size():
			...
			if png.is_empty() or not png.get("ok", false):
				...
				return
			var size: Vector2i = CONTROL_SIZES[ids[i]] if group == "controls" else Vector2i(15 if i == 7 else 16, 30)
