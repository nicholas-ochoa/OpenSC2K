...
static func update_big_park(annual: MicrosimAnnualContext, record_id: int, offset: int) -> void:
	...
	park_visitors = mini(
		park_visitors,
		mini(int(BinaryData.read_u32_be(annual.misc, MISC_NORMAL_POPULATION) / 6), 65000)
	)
	BinaryData.write_u16_be(annual.microsims, offset + 2, park_visitors)
	...
	BinaryData.write_u16_be(annual.microsims, offset + 4, park_count)
	BinaryData.write_u16_be(
		annual.microsims,
		offset + 6,
...
static func update_zoo(annual: MicrosimAnnualContext, record_id: int, offset: int) -> void:
	...
	BinaryData.write_u16_be(annual.microsims, offset + 2, annual.game_random.next_mod(100))
	BinaryData.write_u16_be(annual.microsims, offset + 4, annual.game_random.next_mod(100))
	BinaryData.write_u16_be(annual.microsims, offset + 6, annual.game_random.next_mod(100))
	annual.counts.zoo += 1
...
static func update_statue(annual: MicrosimAnnualContext, record_id: int, offset: int) -> void:
	...
	BinaryData.write_u16_be(annual.microsims, offset + 4, annual.random.next_u15() % 42)
...
static func update_mayor_house(annual: MicrosimAnnualContext, record_id: int, offset: int) -> void:
	...
	if BinaryData.read_u16_be(annual.microsims, offset + 6) != 0:
		BinaryData.write_u16_be(annual.microsims, offset + 6, BinaryData.read_u16_be(annual.microsims, offset + 6) - 1)
...
static func update_library(annual: MicrosimAnnualContext, record_id: int, offset: int) -> void:
	...
	if books > 0 and books < 32000:
		BinaryData.write_u16_be(annual.microsims, offset + 4, books)

	var population := maxi(BinaryData.read_u32_be(annual.misc, MISC_NORMAL_POPULATION), 1)
...
static func update_llamadome(annual: MicrosimAnnualContext, record_id: int, offset: int) -> void:
	...
	if annual.australian_locale:
		BinaryData.write_u16_be(
			...
			+ (BinaryData.read_u32_be(annual.misc, MISC_NORMAL_POPULATION) >> 3)
		)
		annual.microsims[offset + 1] = annual.random.next_u15() & 0x7f
		BinaryData.write_u16_be(annual.microsims, offset + 4, (annual.random.next_u15() & 0x7f) + 10)
	else:
		...
		BinaryData.write_u16_be(annual.microsims, offset + 2, dome_population)
		BinaryData.write_u16_be(
