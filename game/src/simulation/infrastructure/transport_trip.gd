...
static func run(
	...
	if zone < 0 or zone >= DESTINATION_ZONE_MASKS.size():
		return TransportTripResult.failure("zone is outside the supported range")

	if traffic_weight < 0:
		return TransportTripResult.failure("traffic weight cannot be negative")
	...
	if traffic_chunk == null or traffic_chunk.decoded_payload.size() != city.document.decoded_size("XTRF"):
		return TransportTripResult.failure("XTRF is missing or has the wrong size")
	...
	if not TransportTripSearch.valid_inputs(city.buildings, city.zones, city.underground,
		...
		return TransportTripResult.failure("transport input maps have the wrong size")
	...
	if result.reached_destination and not traffic_chunk.set_decoded_payload(traffic):
		return TransportTripResult.failure("cannot store updated XTRF data")
