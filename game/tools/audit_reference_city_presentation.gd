...
func _initialize() -> void:
	...
	assert(FileAccess.file_exists("res://../references/BITMAPS/PRESNTS.BMP"))
	assert(not FileAccess.file_exists("res://../references/BITMAPS/PRESENTS.BMP"))
	var original := CityUiGraphics.load_original("res://../references")
	for id in CityUiGraphics.PRESENTATION_SIZES:
		var bmp := FileAccess.get_file_as_bytes("res://../references/BITMAPS/" + id)
	...
	for id in hashes:
		var bmp := FileAccess.get_file_as_bytes("res://../references/BITMAPS/" + str(id))
		var offset := 14 + bmp.decode_u32(14)
		assert(_sha256(bmp.slice(offset, offset + 1024)) == hashes[id])
		assert(original.presentation_palettes[id].colors == Sc2Palette.load_bmp("res://../references/BITMAPS/" + str(id)).colors)
