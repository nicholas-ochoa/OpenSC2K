...
func _run() -> void:
	...
	main.call("_load_city_unchecked", reference_path)
	...
	for tool in [Vector2i(14, 0), Vector2i(13, 0), Vector2i(3, 2)]:
		...
		start = Time.get_ticks_usec()
		main.call("_refresh_after_city_edit", command)
		var display_ms := (Time.get_ticks_usec() - start) / 1000.0
		start = Time.get_ticks_usec()
		main.call("_play_tool_success_sound", tool.x, tool.y)
