...
func _run() -> void:
	...
	main.full_size_graphics = OS.get_environment("CITY_BENCH_HIRES") != "0"
	...
	for speed in [GameSpeedController.Speed.PAUSED, GameSpeedController.Speed.CHEETAH]:
		...
		for zoom in CityMapControl.ZOOM_LEVELS:
			...
			print("FRAME speed=%d zoom=%.2f hires=%s viewport=%s frames=%d avg_fps=%.2f p95_ms=%.2f p99_ms=%.2f max_ms=%.2f over_60_budget_pct=%.2f" % [speed, zoom, main.full_size_graphics, main.map_view.size, samples.size(), samples.size() * 1000.0 / total, samples[int(samples.size() * 0.95)], samples[int(samples.size() * 0.99)], samples.back(), 100.0 * late / samples.size()])
	main.queue_free()
	await process_frame
	quit()
