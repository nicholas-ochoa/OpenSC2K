...
export function SCEN(data: any) {
  const scen: any = {};

  scen.disaster = {
    type: new DataView(data.slice(4, 6).buffer).getUint16(0),
    x: new DataView(data.slice(6, 7).buffer).getUint8(0),
    y: new DataView(data.slice(7, 8).buffer).getUint8(0),
  };
