import { cells, segments } from '../data';

export function XTXT(data: any) {
  ...
  view.forEach((bits: any, i: any) => {
    ...
    if (xtxt.type == 'sign') {
      xtxt.sign = segments.XLAB[bits];
    }

    if (xtxt.type == 'microsim') {
      xtxt.microsim = {};
      xtxt.microsim.id = bits - 51;
      xtxt.microsim.label = segments.XLAB[bits];

      const xmic = segments.XMIC[xtxt.microsim.id];
